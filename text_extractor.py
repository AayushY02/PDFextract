...
def _write_quality_legend_files(base_dir: Path):
    ...
    jp_text = """品質確認ログ（extraction_quality.csv）について

本プログラムでは、抽出結果の品質を確認するための参考情報として、各ファイルの抽出状態を記録したログファイル（extraction_quality.csv）を出力している。
このログには、抽出テキストの文字構成や構造的特徴を数値化した指標を記録しており、文字化けや構造崩れの兆候を確認するための補助情報として利用できる。

なお、本ログに記録される各指標は抽出結果の状態を把握するための参考情報であり、ログに値が出現した場合でも必ずしも抽出エラーであることを示すものではない。
主な記録項目は以下のとおりである。
文字数：抽出テキストに含まれる文字数
置換文字数：文字化けの兆候となる置換文字（□等）の出現数
制御文字数：改行およびタブ以外の制御文字の数
私用領域文字数：Unicode私用領域（U+E000〜U+F8FF）に含まれる文字数
想定外文字比率：想定される文字種以外の文字の割合
表ブロック数／表比率：PDFのみ対象。表と判定された領域の数および文書全体に対する割合
空番号連続数：②③④…など、内容を伴わない番号行が連続する最大数
文字化け疑い：文字化けの兆候が確認された場合「はい」
表中心：PDFのみ対象。表領域の割合が高い場合「はい」（PDF以外は「対象外」）
構造崩れ疑い：空番号行が一定以上連続する場合「はい」（PDFのみ対象）
問題の種類：以下の分類で記録
形式不一致
無効文字
表が多い
抽出失敗：テキスト抽出に失敗した場合「抽出失敗」と表示
要確認フラグ：問題が検出された場合「☆」を付与（抽出失敗を含む）

なお、各比率指標は「該当文字数 ÷ 文字数」により算出している。
"""
